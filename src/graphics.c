...
uint16_t spr_digits[10][sizeof(tile_4bpp_t) / 2] = {
    {
        0x0002, 0x2000,
        0x0020, 0x0200,
        0x0200, 0x2020,
...
void load_sprites(void) {
    ...
    for(int i = 0; i < 10; i++) {
        volatile uint16_t *digit = (uint16_t *) TILE_MEM[4][DIGIT_OFFSET + i];
        for(int j = 0; j < sizeof(tile_4bpp_t) / 2; j += 2) {
            digit[j] = spr_digits[i][j + 1];
            digit[j + 1] = spr_digits[i][j];
        }
    }
}
